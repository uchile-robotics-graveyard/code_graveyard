...
int main(int argc, char **argv){
	ros::init(argc, argv, "right_planner");
	Uarm brazo = Uarm();
	brazo.spinner.start();
	double qi[4];
	double pos[3];
	double posg[3];
	ros::Publisher cartesian_pub=brazo.nh.advertise<bender_msgs::CartesianInfo>("right_arm/cartesian_info", 1000);
	ros::Publisher grasp_pub=brazo.nh.advertise<bender_msgs::CartesianInfo>("right_arm/grasp_info", 1000);
	ros::Publisher state_pub=brazo.nh.advertise<bender_msgs::StateInfo>("right_arm/state_info", 1000);
	...
	while(ros::ok()){
		...
		if(brazo.apretar)
		{
			...
			ros::ServiceClient client = brazo.nh.serviceClient<bender_srvs::States>("right_arm_joints/states");
