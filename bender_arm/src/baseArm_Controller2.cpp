...
float angleMaxMin[2][6]={{0.63,1.68,2.29,1.61,-0.08,1.1},{-0.82,-1.62,-2.23,-3.35,-1.1,0.08}};
...
/*** servicios ***/
bool setGoal(bender_srvs::setGoalBaseArm::Request &req, bender_srvs::setGoalBaseArm::Response &res);
...
int main(int argc, char **argv){
  ...
  ros::NodeHandle n;
  ros::Publisher baseArmState_pub = n.advertise<bender_msgs::baseArmState>("PosicionActuadoresBaseArm",2);
  ros::Publisher baseArmEfect_pub = n.advertise<bender_msgs::baseEfectorPosition>("PosicionEfectorBaseArm",2);
  ros::Publisher bottomMotors_pub = n.advertise<bender_msgs::Command>("bottom_arm_joints/command",2);
  ros::Publisher grip_pub = n.advertise<bender_msgs::grip>("gripState",1);

  ros::ServiceServer efect_ser = n.advertiseService("setGoalBaseArm", setGoal);
  ...
  ros::ServiceClient getStateClient = n.serviceClient<bender_srvs::States>("bottom_arm_joints/states");//comentar si solo se desea usar el simulador
...
/**/
	...
	ROS_ERROR("Service bottom_arm_joints/states failed while being requested by baseArm_Controller");
    }
/**/
...
//    }
    ...
    //if(((fabs(goal[0]-efector[0])+fabs(goal[1]-efector[1])+fabs(goal[2]-efector[2]))/3.0)>= .5 ){
    if((sqrt(dAngles[0]*dAngles[0] + dAngles[1]*dAngles[1] + dAngles[2]*dAngles[2]))>= .1 ){	
		float norma=sqrt(dAngles[0]*dAngles[0] + dAngles[1]*dAngles[1] + dAngles[2]*dAngles[2]);//medida de distancia del desplazamiento a realizar	
		...
		if( norma>maxDesp){//si el desplazamiento es muy largo se limita
			...
			cmdMsg.positions[0]=armState[0]+maxDesp*dAngles[0]/norma;
			cmdMsg.positions[1]=armState[1]+maxDesp*dAngles[1]/norma;
			cmdMsg.positions[2]=armState[2]+maxDesp*dAngles[2]/norma;
		}
    ...
    //if((fabs(stateUpdate.response.load[4])>0.2 || fabs(stateUpdate.response.load[5])>0.2) && (fabs(goalAngles[4])<=.1 || fabs(goalAngles[5])<=.1) && gripCount<=0.0 || gripState){//condición de agarre
    if(fabs(goalAngles[4])<=.1 || fabs(goalAngles[5])<=.1){

	gripState=true;


	if( (fabs(stateUpdate.response.load[4]) + fabs(stateUpdate.response.load[5]))<0.15 ){
		gripState=false;
	}

	//ROS_INFO("cerrando %.0f %.0f",dAngles[4]*1000,dAngles[5]*1000);
	if( dAngles[4]>=0.05 || dAngles[5]<=-0.05){

		/**Controlador P de la Pinza**/

		float P=0.1;//buscar el valor adecuado y/o aumentar el grado del controlador
		float objLoad=0.5;//corriente a la que se desea ejercer el agarre
		float dLoadIzq=objLoad-fabs(stateUpdate.response.load[4]);
		float dLoadDer=objLoad-fabs(stateUpdate.response.load[5]);
		
		if (dAngles[4]>=0.1 && dAngles[5]<=-0.1 && ~(gripState)){
			P=2*P;
			//ROS_INFO("Apurado");
		}		

		jointStates.Joint5=armState[4]+P*dLoadIzq;
		cmdMsg.positions[4]=armState[4]+P*dLoadIzq;
		jointStates.Joint6=armState[5]-P*dLoadDer;
		cmdMsg.positions[5]=armState[5]-P*dLoadDer;
	       	//ROS_INFO("ganancia %.0f",P);
		//ROS_INFO("izq %.0f der %.0f",dLoadIzq*1000.0,dLoadDer*1000.0);
		/****/

		/* //Comente el controlador y descomente este código al simular
		...
		*/
		...
		cmdMsg.positions[4]=armState[4];
		jointStates.Joint6=armState[5];
		cmdMsg.positions[5]=armState[5];
        	simArmState[4]=armState[4];		
        	simArmState[5]=armState[5];
		//ROS_INFO("detenido");

	}

	gripMsg.somethingGripped=gripState;


    }/**/
    else{//no hay nada en la pinza
	//ROS_INFO("vacia %.0f %.0f",dAngles[4]*1000,dAngles[5]*1000);
...
//Nota: Creo que esto no deviera estar
...
//esta función cambia los valores de la matriz del jacobiano, para actualizarla a los estados O1, O2,O3
	...
	jacobian.at<float>(0,0)= 29.0*sin(O1)*sin(O2)*sin(O3) - 16.0*cos(O2)*sin(O1) - 29.0*cos(O2)*cos(O3)*sin(O1) - 5.3*sin(O1);
	jacobian.at<float>(0,1)=-16.0*cos(O1)*sin(O2) -29.0*cos(O1)*cos(O2)*sin(O3) - 29.0*cos(O1)*cos(O3)*sin(O2);
	jacobian.at<float>(0,2)=-29*cos(O1)*cos(O2)*sin(O3) - 29*cos(O1)*cos(O3)*sin(O2);

	jacobian.at<float>(1,0)= 5.3*cos(O1) + 16.0*cos(O1)*cos(O2) + 29.0*cos(O1)*cos(O2)*cos(O3) - 29.0*cos(O1)*sin(O2)*sin(O3);
	jacobian.at<float>(1,1)=-16.0*sin(O1)*sin(O2) - 29.0*cos(O2)*sin(O1)*sin(O3) - 29.0*cos(O3)*sin(O1)*sin(O2);
	jacobian.at<float>(1,2)=-29.0*cos(O2)*sin(O1)*sin(O3) - 29.0*cos(O3)*sin(O1)*sin(O2);

	jacobian.at<float>(2,0)= 0;
	jacobian.at<float>(2,1)= 16.0*cos(O2) + 29.0*cos(O2)*cos(O3) - 29.0*sin(O2)*sin(O3);
	jacobian.at<float>(2,2)= 29.0*cos(O2)*cos(O3) - 29.0*sin(O2)*sin(O3);
...
bool setGoal(bender_srvs::setGoalBaseArm::Request &req, bender_srvs::setGoalBaseArm::Response &res){
	...
	for(int j=0;j<n;j++){
		...
		int count=0;
		broken[j]=0;
		simulEfect[0][j]=getX(simulState[0][j],simulState[1][j],simulState[2][j]);
		simulEfect[1][j]=getY(simulState[0][j],simulState[1][j],simulState[2][j]);
		simulEfect[2][j]=getZ(simulState[0][j],simulState[1][j],simulState[2][j]);
		while(((fabs(goal[0]-simulEfect[0][j])+fabs(goal[1]-simulEfect[1][j])+fabs(goal[2]-simulEfect[2][j]))/3.0)>= 0.3 ){//dentro de este while se hace el movimiento iterativo hasta la posición deseada

			for(int i=0;i<3;i++){//calculo los deltas para cada angulo
				simulDEfect[i]=(goal[i]-simulEfect[i][j]);
			}

			getControlAction(simulDEfect[0],simulDEfect[1],simulDEfect[2],simulState[0][j],simulState[1][j],simulState[2][j]);//llama a la cinematica inversa, el resultado esta en simulDAngles
			...
			float norma=sqrt(simulDAngles.at<float>(0,0)*simulDAngles.at<float>(0,0) + simulDAngles.at<float>(1,0)*simulDAngles.at<float>(1,0) + simulDAngles.at<float>(2,0)*simulDAngles.at<float>(2,0));//calcula el largo del movimiento a realizar

			//simulo mover el brazo
			if(norma>maxDesp){//si el movimiento es muy largo, se limita
				simulState[0][j]=simulState[0][j]+maxDesp*simulDAngles.at<float>(0,0)/norma;
				simulState[1][j]=simulState[1][j]+maxDesp*simulDAngles.at<float>(1,0)/norma;
				simulState[2][j]=simulState[2][j]+maxDesp*simulDAngles.at<float>(2,0)/norma;
			}
			else{
				simulState[0][j]=simulState[0][j]+simulDAngles.at<float>(0,0);
				simulState[1][j]=simulState[1][j]+simulDAngles.at<float>(1,0);
				simulState[2][j]=simulState[2][j]+simulDAngles.at<float>(2,0);
			}
	...
	for(int j=0;j<n;j++){	
		if(broken[j]==0){		
			if( simulState[0][j]<=angleMaxMin[0][0] && simulState[0][j]>=angleMaxMin[1][0] && simulState[1][j]<=angleMaxMin[0][1] && simulState[1][j]>=angleMaxMin[1][1] &&simulState[2][j]<=angleMaxMin[0][2] && simulState[2][j]>=angleMaxMin[1][2] && (simulEfect[2][j]>0.0 || simulEfect[0][j]>0.0) ){//se revisa que las soluciones cumplan las restricciones de angulos y sobre X y Z para no poder golpear el pioneer		
				float fitness = (armState[0]-simulState[0][j])*(armState[0]-simulState[0][j])+(armState[1]-simulState[1][j])*(armState[1]-simulState[1][j])+(armState[2]-simulState[2][j])*(armState[2]-simulState[2][j]) + alpha*((goal[0]-simulEfect[0][j])*(goal[0]-simulEfect[0][j])+(goal[1]-simulEfect[1][j])*(goal[1]-simulEfect[1][j])+(goal[2]-simulEfect[2][j])*(goal[2]-simulEfect[2][j]));
...
bool setAngles(bender_srvs::setAnglesBaseArm::Request &req, bender_srvs::setAnglesBaseArm::Response &res){
	...
	for(int i=0;i<3;i++){//se revisa que cumplan las restricciones
		if(wannabeGoalAng[i]<angleMaxMin[0][i] && wannabeGoalAng[i]>angleMaxMin[1][i]){
			condition++;
		}		
	}
...
bool grasp(bender_srvs::manipBaseArm::Request &req, bender_srvs::manipBaseArm::Response &res){
	...
	if(fabs(armState[4]+fingerPos)<0.02 || fabs(armState[5]-fingerPos)<0.02 ){
		(res).received=0;
	}
	else{
		(res).received=1;
...
float getX(float O1,float O2,float O3){
	...
	return 5.3*cos(O1) + 16.0*cos(O1)*cos(O2) + 29.0*cos(O1)*cos(O2)*cos(O3) -29.0*cos(O1)*sin(O2)*sin(O3) + 25.0;
}

float getY(float O1,float O2,float O3){

	return 5.3*sin(O1) + 16.0*cos(O2)*sin(O1) + 29.0*cos(O2)*cos(O3)*sin(O1) -29.0*sin(O1)*sin(O2)*sin(O3);
}

float getZ(float O1,float O2,float O3){

	return  16.0*sin(O2) +29.0*cos(O2)*sin(O3) +29.0*cos(O3)*sin(O2) -2.5;
}
