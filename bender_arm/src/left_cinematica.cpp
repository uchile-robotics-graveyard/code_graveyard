...
int main(int argc, char** argv) {
    ...
    ros::init(argc, argv, "l_torque_node");
	ros::NodeHandle nh;
	n = &nh;

	ros::Rate loop_rate(30);

	call_torque = n->serviceClient<bender_srvs::TorqueEnable>("left_arm_joints/torque_enable");
	torque_limit = n->serviceClient<bender_srvs::TorqueLimit>("left_arm_joints/torque_limit");
	torque_on = n -> serviceClient<bender_srvs::Onoff>("left_arm_joints/onoff");
	call_pose = n->advertise<bender_msgs::Command>("left_arm_joints/command",1000);
...
//	double GG[3];
	...
	//ROS_INFO("VARIABLES INICIADAS 1");
...
//    	CenterGravity(C,th,M,6);
//    	printf("CM: X = %f | Y = %f | Z = %f\n",C[0],C[1],C[2]);
//
//    	GetGravity(GG,3,th);
...
//				printf("temp0 = %f | %f | %f \n",temp0.at<double>(0),temp0.at<double>(1),temp0.at<double>(2));
//				printf("temp1 = %f | %f | %f \n",temp1.at<double>(0),temp1.at<double>(1),temp1.at<double>(2));
//				printf("temp2 = %f | %f | %f \n",temp2.at<double>(0),temp2.at<double>(1),temp2.at<double>(2));
//				printf("temp3 = %f | %f | %f \n",temp3.at<double>(0),temp3.at<double>(1),temp3.at<double>(2));
//				printf("temp4 = %f | %f | %f \n",temp4.at<double>(0),temp4.at<double>(1),temp4.at<double>(2));
//				printf("temp5 = %f | %f | %f \n",temp5.at<double>(0),temp5.at<double>(1),temp5.at<double>(2));
...
void LeerAngulo(double th[])
{
	...
	ros::ServiceClient client = n->serviceClient<bender_srvs::States>("left_arm_joints/states");
...
//	torque[5] = std::cos(th[5]+th[3]-pi/2)*M[6]*L[5]*g;
	...
	double MaxTorque106 = 2300;
	double MaxTorque64 = 570;
	double MaxTorque28 = 400;
...
//	printf("X = %f", x);
	ty = masa * (x*grav[2] - erre[2]*grav[0]);
	tz = masa * (x*grav[1] - erre[1]*grav[0]);
...
//	joint_select[4] = true;
	...
	{
		LimitarTorqueMax(left_limite,1);
...
//	 CODO Y
	...
	{
		LimitarTorqueMax(left_limite,4);
		SetPosicion(th, joint_select);
	}
...
//	}
	...
	// MUNECA Y
	if (carga[5] > torque[5] + eps6 ) // hacia abajo
	{
		LimitarTorqueMax(left_limite,6);
		SetPosicion(th, joint_select);
	}
	if(carga[5] < torque[5] - eps6) // hacia arriba
	{
		LimitarTorqueMax(left_limite,6);
		SetPosicion(th, joint_select);
	}
...
bool LimitarTorqueMax(double limiteTorque[], int joint)
{
	...
	{
	case 1: tl.request.motor_name = "hombro_1"; break;
	case 2: tl.request.motor_name = "hombro_2"; break;
	case 3: tl.request.motor_name = "hombro_3"; break;
	case 4: tl.request.motor_name = "codo_1"; break;
	case 5: tl.request.motor_name = "codo_2"; break;
	case 6: tl.request.motor_name = "muneca"; break;
	}
	...
	{
		return res = false;
		ROS_INFO("Failed to call service: left_arm_joints/torque_limit");
	}
	return res;
}
