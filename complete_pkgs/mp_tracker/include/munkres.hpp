...
bool
Munkres<FloatMatrix,IntMatrix>::find_uncovered_in_matrix(double item, int &row, int &col) {
  for ( row = 0 ; row < rows(matrix) ; row++ )
    if ( !row_mask[row] )
      for ( col = 0 ; col < cols(matrix) ; col++ )
        if ( !col_mask[col] )
...
template<typename FloatMatrix, typename IntMatrix>
bool 
Munkres<FloatMatrix,IntMatrix>::pair_in_list(const std::pair<int,int> &needle, const std::list<std::pair<int,int> > &haystack) {
...
template<typename FloatMatrix, typename IntMatrix>
int 
Munkres<FloatMatrix,IntMatrix>::step1(void) {
  for ( int row = 0 ; row < rows(matrix) ; row++ )
    for ( int col = 0 ; col < cols(matrix) ; col++ )
      if ( matrix(row,col) == 0 ) {
        bool isstarred = false;
        for ( int nrow = 0 ; nrow < rows(matrix) ; nrow++ )
        ...
        if ( !isstarred ) {
          for ( int ncol = 0 ; ncol < cols(matrix) ; ncol++ )
...
template<typename FloatMatrix, typename IntMatrix>
int 
Munkres<FloatMatrix,IntMatrix>::step2(void) {
  int rows_ = rows(matrix);
  int cols_ = cols(matrix);
  int covercount = 0;
  ...
  int k = std::min(rows(matrix), cols(matrix));
...
template<typename FloatMatrix, typename IntMatrix>
int 
Munkres<FloatMatrix,IntMatrix>::step3(void) {
  /*
  Main Zero Search
...
template<typename FloatMatrix, typename IntMatrix>
int 
Munkres<FloatMatrix,IntMatrix>::step4(void) {
  int rows_ = rows(matrix);
  int cols_ = cols(matrix);
  ...
  */
  ...
  for ( int row = 0 ; row < rows(mask_matrix) ; row++ )
    for ( int col = 0 ; col < cols(mask_matrix) ; col++ )
      if ( mask_matrix(row,col) == PRIME )
...
template<typename FloatMatrix, typename IntMatrix>
int 
Munkres<FloatMatrix,IntMatrix>::step5(void) {
  int rows_ = rows(matrix);
  int cols_ = cols(matrix);
  /*
...
template<typename FloatMatrix, typename IntMatrix>
void 
Munkres<FloatMatrix,IntMatrix>::solve(FloatMatrix &m) {
  ...
  zero(mask_matrix, rows(matrix), cols(matrix));

  row_mask = new bool[rows(matrix)];
  col_mask = new bool[cols(matrix)];
