...
class RotateHead(smach.State):
    ...
    def __init__(self):
        ...
        self.head_angles = [0, 20, 35, 20, 0, -20, -35, -20]
...
class ExtractPose(smach.State):
    ...
    def __init__(self):
        ...
        self.cam_model = rospy.get_param(
            '/bender/sensors/' + self.cam_name + '/face_model',
            default_cam_model
        )
    ...
    def execute(self, userdata):
        ...
        while (not rospy.is_shutdown()):
            ...
            try:
                ...
                transform_res = self.transform_client(pose_in=face, frame_out='/map')
                break
...
def getInstance():
    ...
    sm = smach.StateMachine(
        outcomes=['succeeded','aborted','preempted'],
        output_keys=['sm_face_pose']
    )

    sm.userdata.sm_face_pose = PoseStamped()
