...
class OctomapManager(object):
  ...
  def __init__(self):
    # Obtener servicio
    try:
      rospy.wait_for_service('/manage_octomap', timeout=5)
    except:
...
class End(smach.State):
  ...
  def __init__(self,limb):
    smach.State.__init__(self, outcomes=['succeeded','aborted','preempted'],
                              input_keys=['lr_arm'])
    self.arms = limb
    self.carrypos = [0.481, 0.161, 1.308, 1.784, -0.542, -0.271]
  ...
  def execute(self, userdata):
    ...
    self.limb.arm.move_joint(self.carrypos)
    self.limb.arm.wait()
...
def getInstance():
  ...
  sm.userdata.trayectory_name_after_close = ['premanip_1']
  ...
  with sm:
      ...
      smach.StateMachine.add('CLOSE_GRIP', ArmStates.closeGripper(arms),
                           transitions={'notgrabbing':'APOLOGIZE','stalled':'SERVOING_Z','aborted':'MOVE_NAMED_NOT_GRABBED'},
      ...
      smach.StateMachine.add('SERVOING_Z',ArmStates.Servoing(arms),
                            transitions={'succeeded':'CARRY_POSITION'},
                            remapping={'dz':'dz','dy':'dy','dx':'dx','lr_arm':'lr_arm'}
                            )
